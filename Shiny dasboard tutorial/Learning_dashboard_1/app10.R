## R ShinyTutorial - 10. Adding a new box to shiny dashboard and putting controls in it | - R Programming Tutorials

## box - status parameter for shiny helps shiny to know about status of box
# it gives color at border level and at header level based on how we specify
# Eg- 'danger' has Red color, 'success' has Green ---- search -> ?validStatuses
  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "dashboard",
              fluidRow(
                box(title = 'Histogram of faithful', status = 'primary', solidHeader = TRUE, plotOutput("histogram")),
                # can also tell users details about how to use controls br() is an html tag for break
                box(title = 'Dashboard controls', status = 'warning', solidHeader = TRUE, 
